...
toast.success('login successfully',{postion:'top-center', theme:'dark'})
ctx.loginUser({login : true , email:userExists.email})
navigate('/')
 }
 else{
  toast.error('wrong password', {postion:'top-center', theme:'dark'})
 }
}
else{
  toast.error('user not found please signup', {postion:'top-center', theme:'dark'})
}
...
<form className="max-w-sm mx-auto bg-slate-600 p-10 rounded-md  mt-36 ">
  <h1 className='text-white text-2xl my-3'>Login Form</h1>
  <div className="mb-5">
  ...
  <p className='text-center my-3 text-white'>Don't have any account ? <Link  className='bg-blue-700' to = '/register'>Signup</Link></p>
</form>
